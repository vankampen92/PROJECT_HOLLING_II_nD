...
/* These functions allocate, initialize and free a number of local communities,
   (or patches), which make up our total patch system or metacommunity.
   The network that interconnects the local communities can be defined with total 
   generality. Here, you will only find the implementation of a regular squared 
   grid. 

   Notice that here you will also find the definitions of functions that are 
   required to implement fast stochastic Gillespi-based algorithms for spatial
   extended systems, such as, the binary tree based (direct method) and the 
   priority queu based (Gibson and Bruck)
  
   These functions are general and can be used regardless the individual processes
   (within each local community) that every particular model may require. 
*/
			                         ...
			                         Parameter_Model * P )
{
  ...
  /* This optimization can be implemented for any model. 
   * In this example, it has been implemented for: 
     
     . MODEL=DIFFUSION_HII_nD           TYPE_of_MODEL = 16
     
     Therefore, here I will make sure that this is the model at work
     when the program comes to this point. 
  */
  if(P->TYPE_of_MODEL == 16){
