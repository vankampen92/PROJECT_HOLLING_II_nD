...
* Summary of set up:
	...
	A graphical output representing the temporal evolution of the marginals from the whole probability distribution temporal evolution calculated by both numberically integrating the master equation and through the analytic result (a time-dependent multinomial as reported in the companion paper) is given here (here input arguments: -HS 4 -n4 -G0 2 -G1 2, this is, for 4 resource types):
